...
result=$(echo "from django.contrib.auth import get_user_model; User = get_user_model(); print(User.objects.filter(user
name='$PATCHMAN_USERNAME').count()>0)" | patchman-manage shell | tail -n 1)
...
exec gunicorn patchman.wsgi --bind $PATCHMAN_HOST:$PATCHMAN_PORT
